...
//data run
class NiskTest : public ::testing::Test {
  protected:

  
  static void SetUpTestSuite()  {
    auto [v2,n_int,d_int] = parse_uint8bin("/Users/andrewbrady/Documents/GitHub/parlaykmeans/Data/base.1B.u8bin.crop_nb_1000");//parse_fbin("/ssd1/data/text2image1B/base.1B.fbin.crop_nb_1000");
  n = (size_t) n_int;
   d = (size_t) d_int;
   v=v2;
  k = 10;
  ...
  LazyStart<uint8_t> init; //TODO change back to float
  init(v,n,d,k,c,asg,*D);

  std::cout << "madre it there" << std::endl;
  
  NiskKmeans<uint8_t> nie; //TODO change back
  ...
  static float* c;
  static uint8_t* v; //TODO change back to float
  static size_t n;
  static size_t k;
  static size_t d;
...
float* NiskTest::c = nullptr;
uint8_t* NiskTest::v = nullptr;
size_t NiskTest::n = 0;
size_t NiskTest::k = 0;
size_t NiskTest::d = 0;
size_t NiskTest::max_iter = 0;
Distance* NiskTest::D = nullptr;
float NiskTest::epsilon = 0;
size_t* NiskTest::asg = nullptr;

//Tests that the values in num_members make sense,
//And that points are assigned to a valid center
TEST_F(NiskTest,MemberCheck) {
...
TEST_F(NiskTest,CenterCentroids) {
