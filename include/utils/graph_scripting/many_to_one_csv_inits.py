...
for i in range(num_diff_methods):
  ...
  for j in range(int(input("How many k values run on this method? "))):
...
for i in range(num_ks):
  ...
  for j in range(num_diff_methods):
    if (i < len(log_objects_list[j])):
      row_data.append(log_objects_list[j][i].total_time)
    else:
      row_data.append(-1) #if not run for that k
  for j in range(num_diff_methods):
    if (i < len(log_objects_list[j])):
